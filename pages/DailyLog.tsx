  ...
  return (
    <div className="bg-white p-2 font-sans">
      ...
      <table className="w-full border-collapse border border-black text-[9px] text-center leading-tight">
        <thead>
          <tr>
            <th className="p-[1px]" rowSpan={2} style={{ width: '25px' }}>NO</th>
            <th className="p-[1px]" rowSpan={2} style={{ width: '140px' }}>ADI SOYADI</th>
            <th className="p-[1px]" rowSpan={2} style={{ width: '70px' }}>ARAÇ PLAKASI</th>
            <th className="p-[1px] h-20 w-6" rowSpan={2}><div className="vertical-text-header">İLK OKUL</div></th>
            <th className="p-[1px] h-20 w-6" rowSpan={2}><div className="vertical-text-header">ORTA OKUL</div></th>
          ...
          <tr>
            {weekDates.map((_, index) => (
              <React.Fragment key={index}>
                <th className="p-[1px] h-24 w-5 font-normal"><div className="vertical-text-header">TESLİM ALINAN</div></th>
                <th className="p-[1px] h-24 w-5 font-normal"><div className="vertical-text-header">TESLİM EDİLEN</div></th>
                <th className="p-[1px] w-14 font-bold text-[8px]">ŞOFÖR İMZA SABAH</th>
                <th className="p-[1px] w-14 font-bold text-[8px]">ŞOFÖR İMZA AKŞAM</th>
              </React.Fragment>
            ))}
          </tr>
        </thead>
        <tbody>
          {drivers.map((driver, index) => (
            <tr key={driver.id} className="h-8"> {/* Reduced fixed height */}
              <td className="font-bold p-[1px]">{index + 1}</td>
              <td className="font-bold px-1 whitespace-nowrap text-left p-[1px]">{driver.name}</td>
              <td className="font-bold whitespace-nowrap p-[1px]">{driver.plateNumber || '-'}</td>
              <td></td><td></td>
              {Array.from({ length: 5 }).map((_, i) => (<React.Fragment key={i}><td /><td /><td /><td /></React.Fragment>))}
            </tr>
          ))}
          <tr><td colSpan={3 + 2 + (5 * 4)} className="h-2 bg-slate-100"></td></tr>
          <tr className="h-8">
            <td className="text-right pr-2 font-bold" colSpan={3}>TOPLAM</td>
            <td className="bg-slate-200" colSpan={2}></td>
            {weekDates.map((_, i) => {
              const teachers = (settings.dutyTeachers[i] || '').split(/[\n,]/).map(t => t.trim()).filter(t => t);
              const displayTeachers = teachers.length > 0 ? teachers : [];

              return (
                <td key={i} colSpan={4} className="text-center align-bottom pb-0.5 font-bold">
                  {displayTeachers.length > 0 ? (
                    displayTeachers.map((teacher, idx) => (
                      <div key={idx} className={`${idx > 0 ? 'mt-1' : ''}`}>
                        <div className="text-[8px] leading-none whitespace-nowrap overflow-hidden text-ellipsis px-0.5">{teacher}</div>
                        <div className="border-b border-black border-dotted w-[90%] mx-auto mt-0.5 dotted-line"></div>
                      </div>
                    ))
                  ) : (
                    <div className="py-1">
                      <div className="text-[8px] text-slate-300">Nöbetçi</div>
                      <div className="border-b border-black border-dotted w-[80%] mx-auto mt-2 dotted-line"></div>
                    </div>
                  )}
                </td>
              )
