...
export const MapView: React.FC<MapViewProps> = ({ students, drivers, onUpdateStudent, settings, onUpdateSettings }) => {
  ...
  const villageMarkers = useMemo(() => {
      ...
      const uniqueVillages = Array.from(new Set(students.map(s => s.village).filter(Boolean)));
      return (uniqueVillages as string[]).map((village: string) => {
          let position: [number, number];
          if (settings.villageLocations && (settings.villageLocations as any)[village]) {
              position = [(settings.villageLocations as any)[village].lat, (settings.villageLocations as any)[village].lng];
          } else {
