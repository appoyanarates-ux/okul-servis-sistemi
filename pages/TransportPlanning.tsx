...
import { saveTransportPlanData, loadTransportPlanData } from '../services/storage';
...
// --- MAIN COMPONENT ---
export const TransportPlanning: React.FC<TransportPlanningProps> = ({ students, settings }) => {
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    value={editingRow.distance}
                    onChange={(e) => setEditingRow({ ...editingRow, distance: Number(e.target.value) })}
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Araç Sayısı</label>
                  <input type="number" className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" value={editingRow.vehicleCount} onChange={(e) => setEditingRow({ ...editingRow, vehicleCount: Number(e.target.value) })} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Araç Kapasitesi</label>
                  <input type="number" className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" value={editingRow.capacity} onChange={(e) => setEditingRow({ ...editingRow, capacity: Number(e.target.value) })} />
                </div>
                <div>
              ...
              <div className="grid grid-cols-2 gap-4 bg-slate-50 p-3 rounded-lg border border-slate-100">
                <div>
                  ...
                  <input type="number" className="w-full px-2 py-1.5 bg-white border border-slate-200 rounded text-sm focus:ring-1 focus:ring-blue-500 outline-none" value={editingRow.dailyCost} onChange={(e) => setEditingRow({ ...editingRow, dailyCost: Number(e.target.value) })} />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 mb-1">Yıllık Maliyet (TL)</label>
                  <input type="number" className="w-full px-2 py-1.5 bg-white border border-slate-200 rounded text-sm focus:ring-1 focus:ring-blue-500 outline-none" value={editingRow.yearlyCost} onChange={(e) => setEditingRow({ ...editingRow, yearlyCost: Number(e.target.value) })} />
                </div>
              </div>
            </div>
            <div className="p-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
              <button onClick={() => { setEditingRow(null); if (document.activeElement instanceof HTMLElement) document.activeElement.blur(); }} className="px-4 py-2 text-slate-600 font-medium hover:bg-slate-200 rounded-lg">İptal</button>
