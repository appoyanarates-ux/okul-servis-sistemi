...
export const Students: React.FC<StudentsProps> = ({ students, drivers, onDelete, onBulkDelete, onBulkUpdate, onUpdate, onAdd, settings, onUpdateSettings }) => {
  ...
  const handleBatchMove = (e: React.FormEvent) => {
    e.preventDefault();
    if (!targetClassName || selectedIds.size === 0) return;

    const studentsToUpdate = students.filter(s => selectedIds.has(s.id)).map(s => ({
      ...s,
    ...
    setTargetClassName('');
    setSelectedIds(new Set());
    alert(`${studentsToUpdate.length} öğrenci başarıyla ${targetClassName} sınıfına taşındı.`);
  };

  const handlePromoteAll = () => {
    if (!students || students.length === 0) {
      alert('Sistemde öğrenci bulunamadı.');
      return;
    }

    if (!window.confirm('TÜM öğrencileri bir üst sınıfa taşımak istediğinize emin misiniz? (12. sınıflar MEZUN edilecektir)')) return;

    const promoteClass = (className: string) => {
    ...
    const updatedStudents = students.map(s => ({
      ...s,
      className: promoteClass(s.className)
    }));

    onBulkUpdate(updatedStudents);
    alert('Tüm öğrenciler başarıyla bir üst sınıfa taşındı. Uygulama verileri güncellendi.');
  };

  const handleGraduateGrade = (e: React.FormEvent) => {
    e.preventDefault();
    const gradeToGraduate = graduateGrade;
    const targetStudents = students.filter(s => s.className.trim().startsWith(gradeToGraduate));

    if (targetStudents.length === 0) {
      alert(`${gradeToGraduate}. sınıf seviyesinde öğrenci bulunamadı.`);
      return;
    }

    if (!window.confirm(`${targetStudents.length} adet ${gradeToGraduate}. sınıf öğrencisini mezun etmek istediğinize emin misiniz?`)) return;

    const updatedStudents = targetStudents.map(s => ({
      ...s,
      className: 'MEZUN'
    }));
  ...
  const handleDeleteClass = () => {
    ...
    const classNameToDelete = classFilter;
    const classStudents = students.filter(s => s.className === classNameToDelete);

    if (classStudents.length === 0) {
      alert('Bu sınıfta silinecek öğrenci bulunamadı.');
      return;
    }

    if (window.confirm(`"${classNameToDelete}" sınıfındaki TÜM (${classStudents.length}) öğrencileri silmek istediğinize emin misiniz? Bu işlem geri alınamaz.`)) {
      onBulkDelete(classStudents.map(s => s.id));
      setClassFilter('all');
      alert(`"${classNameToDelete}" sınıfı ve içindeki tüm öğrenciler başarıyla silindi.`);
    }
  };
    ...
    <div className="space-y-8 animate-fade-in">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
                {selectedIds.size > 0 && (
                    <div className="flex items-center gap-2 animate-fade-in">
                        <button
                            ...
                            className="flex items-center gap-2 px-4 py-2.5 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded-xl transition-colors font-medium"
                            title="Seçilenleri Sınıfa Taşı"
                        >
                            <ArrowRightLeft size={18} />
                            <span className="hidden sm:inline">Sınıfa Taşı</span>
                        </button>
                        <button
                            onClick={handleBulkDeleteClick}
                            className="flex items-center gap-2 px-4 py-2.5 bg-red-50 text-red-600 hover:bg-red-100 rounded-xl transition-colors font-medium"
                        >
                            <Trash2 size={18} />
                ...
                <div className="relative group">
                    <button className="flex items-center gap-2 px-4 py-2.5 bg-white text-slate-700 hover:bg-slate-50 rounded-xl border border-slate-200 transition-all font-medium shadow-sm">
                        <CheckSquare size={18} className="text-slate-400" />
                        <span>Toplu İşlemler</span>
                    </button>
                    <div className="absolute right-0 mt-2 w-56 bg-white border border-slate-200 rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-20 overflow-hidden">
                        <button onClick={handlePromoteAll} className="w-full text-left px-4 py-3 text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2 border-b border-slate-100">
            ...
            <div className="flex items-center gap-2 w-full md:w-auto p-1">
                ...
                {classFilter !== 'all' && (
                    <button
                        ...
                        className="flex items-center gap-2 px-3 py-2.5 bg-red-50 text-red-600 hover:bg-red-100 rounded-xl transition-colors font-medium text-sm"
                        title="Bu Sınıfı Tamamen Sil"
                    >
                        <Trash2 size={16} />
                        <span className="hidden lg:inline">Sınıfı Sil</span>
                    </button>
                )}
      ...
      {isGraduateModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
             <div className="p-4 border-b border-slate-100 flex justify-between items-center">
                <h3 className="font-bold text-lg">Sınıf Mezun Et</h3>
                <button onClick={() => setIsGraduateModalOpen(false)}><X className="text-slate-400 hover:text-slate-600" /></button>
             </div>
             <form onSubmit={handleGraduateGrade} className="p-6 space-y-4">
                <div>
                    <p className="text-sm text-slate-500 mb-4">
                        Hangi seviyedeki tüm öğrencileri mezun etmek istiyorsunuz? (Örn: 8 yazarsanız 8/A, 8/B vb. tümü mezun edilir)
                    </p>
                    <label className="block text-sm font-medium text-slate-700 mb-1">Mezun Edilecek Sınıf Seviyesi</label>
                    <select
                        required
                        className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
                        value={graduateGrade}
                        onChange={(e) => setGraduateGrade(e.target.value)}
                    >
      ...
      {isBatchMoveModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
             <div className="p-4 border-b border-slate-100 flex justify-between items-center">
                <h3 className="font-bold text-lg">Seçilenleri Sınıfa Taşı</h3>
                <button onClick={() => setIsBatchMoveModalOpen(false)}><X className="text-slate-400 hover:text-slate-600" /></button>
             </div>
             <form onSubmit={handleBatchMove} className="p-6 space-y-4">
                <div>
                    <p className="text-sm text-slate-500 mb-4">
                        Seçili <strong className="text-slate-800">{selectedIds.size}</strong> öğrenciyi aşağıdaki sınıfa taşımak istediğinize emin misiniz?
                    </p>
                    <label className="block text-sm font-medium text-slate-700 mb-1">Yeni Sınıf / Şube</label>
                    <input
                        required
                        type="text"
                        list="classOptions"
                ...
                <div className="pt-4 flex justify-end gap-2">
                    <button type="button" onClick={() => setIsBatchMoveModalOpen(false)} className="px-4 py-2 text-slate-600 bg-slate-100 rounded-lg hover:bg-slate-200 font-medium">İptal</button>
                    <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium">Taşı</button>
                </div>
             </form>
          </div>
        </div>
