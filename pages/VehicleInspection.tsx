...
const InspectionFormPage: React.FC<{
    ...
    const signatories = [
        // Vice Principals
        settings.vicePrincipal1 ? { name: settings.vicePrincipal1, title: 'Müdür Yardımcısı' } : null,
        settings.vicePrincipal2 ? { name: settings.vicePrincipal2, title: 'Müdür Yardımcısı' } : null,
        settings.vicePrincipal3 ? { name: settings.vicePrincipal3, title: 'Müdür Yardımcısı' } : null,
        settings.vicePrincipal4 ? { name: settings.vicePrincipal4, title: 'Müdür Yardımcısı' } : null,
        // Principals
        settings.principalName ? { name: settings.principalName, title: 'Okul Müdürü' } : null,
        settings.principalName2 ? { name: settings.principalName2, title: 'Okul Müdürü' } : null,
    ...
    return (
        <div className="bg-white p-8 font-['Times_New_Roman'] text-black text-[11px] leading-tight min-h-[297mm] relative box-border">
            ...
            <table className="w-full mb-1 text-[11px] border-collapse border border-black">
                <tbody>
                    <tr>
                        <td className="font-bold p-1 w-[25%] bg-gray-100 border border-black">ARACIN MODEL YILI</td>
                        <td className="p-1 w-[25%] text-center border border-black">{driver?.vehicleYear || ''}</td>
                        <td className="font-bold p-1 w-[25%] bg-gray-100 border border-black">TELEFON GSM</td>
                        <td className="p-1 w-[25%] text-center border border-black">{driver?.phone}</td>
                    </tr>
                    <tr>
                        <td className="font-bold p-1 bg-gray-100 border border-black">ARACIN PLAKASI</td>
                        <td className="p-1 font-bold text-center border border-black">{driver?.plateNumber}</td>
                        <td className="font-bold p-1 bg-gray-100 border border-black">ÖĞRENCİ SAYISI</td>
                        <td className="p-1 text-center font-bold border border-black">{studentCount}</td>
                    </tr>
                    <tr>
                        <td className="font-bold p-1 bg-gray-100 border border-black">SÜRÜCÜ BELGESİNİN ALINDIĞI YIL VE SINIFI</td>
                        <td className="p-1 text-center border border-black">{extraData.licenseInfo}</td>
                        <td className="font-bold p-1 bg-gray-100 text-[10px] border border-black">OKULA KURUMA GELEN ARAÇ SAYISI</td>
                        <td className="p-1 text-center border border-black">{totalVehicles}</td>
                    </tr>
                    <tr>
                        <td className="font-bold p-1 bg-gray-100 border border-black">ŞOFÖRÜN ADI/SOYADI</td>
                        <td className="p-1 font-bold text-center border border-black">{driver?.name}</td>
                        <td className="font-bold p-1 bg-gray-100 border border-black">DENETLEME TARİHİ</td>
                        <td className="p-1 text-center border border-black">{extraData.inspectionDate}</td>
                    </tr>
                    <tr>
                        <td className="font-bold p-1 bg-gray-100 border border-black">T.C.KİMLİK NO</td>
                        <td className="p-1 text-center border border-black" colSpan={3}>{driver?.tcNumber}</td>
                    </tr>
                    <tr>
                        <td className="font-bold p-1 bg-gray-100 border border-black">ARACIN GÜZERGAHI</td>
                        <td className="p-1 text-[10px] text-center border border-black" colSpan={3}>{formattedRoute}</td>
                    </tr>
                </tbody>
            </table>
            ...
            <table className="w-full text-[10px] border-collapse border border-black">
                <thead>
                    <tr className="bg-gray-100 font-bold text-center">
                        <td className="p-1 text-left border border-black">DENETLEME KONULARI</td>
                        <td className="p-1 w-10 border border-black">EVET</td>
                        <td className="p-1 w-10 border border-black">HAYIR</td>
                        <td className="p-1 w-24 border border-black">AÇIKLAMALAR</td>
                    </tr>
                </thead>
                <tbody>
                    {INSPECTION_ITEMS.map((item, idx) => (
                        <tr key={idx}>
                            <td className="p-1 px-2 border border-black">{item}</td>
                            <td className="border border-black text-center font-bold text-xs">{answers[idx] === 'yes' ? 'X' : ''}</td>
                            <td className="border border-black text-center font-bold text-xs">{answers[idx] === 'no' ? 'X' : ''}</td>
...
export const VehicleInspection: React.FC<VehicleInspectionProps> = ({ drivers, students, settings }) => {
    ...
    const exportToExcel = () => {
        ...
        if (driversToExport.length === 0) {
            setMsg({ type: 'error', text: "Lütfen en az bir şoför seçiniz." });
            ...
            {msg && (
                <div className={`fixed bottom-6 right-6 z-50 flex items-center gap-3 ${msg.type === 'success' ? 'bg-green-600' : 'bg-red-600'} text-white px-6 py-4 rounded-xl shadow-2xl animate-slide-in-up`}>
                    <div className="bg-white/20 p-2 rounded-full">
                        {msg.type === 'success' ? <CheckCircle size={24} /> : <XCircle size={24} />}
                    </div>
                    <div>
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        <input
                                            type="text"
                                            autoFocus
                                            className="w-full pl-8 pr-3 py-1.5 border border-slate-300 rounded text-sm focus:ring-1 focus:ring-blue-500 outline-none"
